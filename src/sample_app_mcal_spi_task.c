...
FUNC (Std_ReturnType, SAMPLE_APP_CODE) SampleAppSpiTask(P2VAR(SampleAppData_T, AUTOMATIC, SAMPLE_APP_VAR) pstSampleAppData)
{
    ...
    switch(pstSampleAppData->stSpiData.ucState)
    {
        ...
        {
            stdRet = Spi_WriteIB( SpiConf_SpiChannel_SpiChannel_0, ucSource);
            if (stdRet != E_OK)
            {
                CONSOLE_MESSAGE("SPI Error %d returned by Spi_WriteIB( SpiConf_SpiChannel_SpiChannel_0, ucTemp)", stdRet);
                /*CONSOLE_MESSAGE("SPI state-machine state:%d", pstSampleAppData->stSpiData.ucState);*/
                pstSampleAppData->stSpiData.ucState = SPI_ERROR_STATE;
                return (E_OK);
            }
        ...
        {
            stdRet = Spi_AsyncTransmit(SpiConf_SpiSequence_SpiSequence_0);
            if (stdRet != E_OK)
            {
                CONSOLE_MESSAGE("SPI Error %d returned by Spi_AsyncTransmit(SpiConf_SpiSequence_SpiSequence_0)", stdRet);
                /*CONSOLE_MESSAGE("SPI state-machine state:%d", pstSampleAppData->stSpiData.ucState);*/
                pstSampleAppData->stSpiData.ucState = SPI_ERROR_STATE;
                return (E_OK);
            }
        ...
        {
            stdRet =  Spi_GetSequenceResult(SpiConf_SpiSequence_SpiSequence_0);
            ...
            {
                stdRet = Spi_ReadIB( SpiConf_SpiChannel_SpiChannel_0, ucDest);
                if (stdRet != E_OK)
                {
                    CONSOLE_MESSAGE("SPI Error %d returned by Spi_ReadIB( SpiConf_SpiChannel_SpiChannel_0, ucDest)", stdRet);
                    /*CONSOLE_MESSAGE("SPI state-machine state:%d", pstSampleAppData->stSpiData.ucState);*/
                    pstSampleAppData->stSpiData.ucState = SPI_ERROR_STATE;
                    return (E_OK);
                }
            ...
            else
            {
                ...
                CONSOLE_MESSAGE("SPI Error transmission");
                /* change the state */
                pstSampleAppData->stSpiData.ucState = SPI_ERROR_STATE;
                return (E_OK);
            }
        ...
        {
            CONSOLE_MESSAGE("SPI Error not initialized");
            pstSampleAppData->stSpiData.ucState = SPI_ERROR_STATE;
            return (E_OK);
        }
        /*break;*/
