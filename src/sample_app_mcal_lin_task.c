...
#define LIN_GETSTATUS_TIMEOUT         0x4000UL
...
FUNC (Std_ReturnType, SAMPLE_APP_CODE) SampleAppLinTask(P2VAR(SampleAppData_T, AUTOMATIC, SAMPLE_APP_VAR) pstSampleAppData)
{
    ...
    switch(pstSampleAppData->stLinData.ucState)
    {
        ...
        case LIN_TRANSMIT_STATE:
        {
            ...
            stdRet = Lin_SendFrame(LIN_LOGICAL_CHANNEL, &pstSampleAppData->stLinData.tosend_header);
            if (stdRet != E_OK)
            {
                CONSOLE_MESSAGE("LIN Error %d returned by Lin_SendFrame\n", stdRet);
                pstSampleAppData->stLinData.ucState = LIN_ERROR_STATE;
                return (E_OK);
            }

            /* we've sent the frame and the response and now go to TX pending state */
        ...
        case LIN_TX_PENDING_STATE:
        {
            ...
            if ((stdRet == LIN_OPERATIONAL) || (LIN_TX_OK == stdRet))
            {
